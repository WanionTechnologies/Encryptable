...
class EncryptableListFieldAspect {
    ...
    fun beforeFieldGet(joinPoint: JoinPoint) {
        ...
        val encryptableListField = Encryptable.getMetadataFor(targetEncryptable).encryptableListFields[fieldName] ?: return

        val list = encryptableListField.get(targetEncryptable)
        if (list is EncryptableList<*>) return

        val encryptableList = EncryptableList(fieldName, targetEncryptable)
        encryptableListField.set(targetEncryptable, encryptableList)
    }
    ...
    fun aroundEncryptableListFieldSet(joinPoint: ProceedingJoinPoint): Any? {
        ...
        val metadata = Encryptable.getMetadataFor(targetEncryptable)

        val encryptableListFieldMap = metadata.encryptableListFields
        ...
        if (Encryptable.isNew(targetEncryptable)) {
            val encryptableListField = metadata.encryptableListFields[fieldName] ?: throw IllegalStateException("Field '$fieldName' not found in metadata.")
            val newValue = joinPoint.args.firstOrNull() ?: return joinPoint.proceed()
            if (newValue is List<*>) {
                // Create EncryptableList with the starting list items
                val encryptableList = EncryptableList(fieldName, targetEncryptable, newValue)
                encryptableListField.set(targetEncryptable, encryptableList)
